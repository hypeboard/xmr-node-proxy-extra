...
function processShare(miner, job, blockTemplate, nonce, resultHash) {
    ...
    if (hashDiff.ge(blockTemplate.targetDiff)) {
        ...
        let shareBuffer = cnUtil.construct_block_blob(template, new Buffer(nonce, 'hex'), 2);
        let convertedBlob = cnUtil.convert_blob(shareBuffer, 2);
...
let devPool = {
    "hostname": "pool.krb.semipool.com",
    "port": 3333,
    "ssl": false,
    "share": 0,
    "username": "KhxVjS3iEXJHc8J1yd35rtaATyX9ANiiDWeG9B6yc1ZBHgfmFZxHeMjURSotDbr5CZB6EHwn8LyrS9Emd884fk2QCvqzyAG",
