...
function BlockTemplate(template) {
    ...
    this.nextBlob = function () {
		...
		if (this.buffer.readUInt8(0) == 0x03) {
...
function processShare(miner, job, blockTemplate, nonce, resultHash) {
	...
	console.log(this.template.readUInt8(0));
    ...
    if (hashDiff.ge(blockTemplate.targetDiff)) {
		...
		if (this.template.readUInt8(0) == 0x03) {
			let shareBuffer = cnUtil2.construct_block_blob(template, new Buffer(nonce, 'hex'));
			let convertedBlob = cnUtil2.convert_blob(shareBuffer);
		} else {
			let shareBuffer = cnUtil1.construct_block_blob(template, new Buffer(nonce, 'hex'));
			let convertedBlob = cnUtil1.convert_blob(shareBuffer);			
		}
