...
const cnUtil1 = require('cryptonote-util');
const cnUtil2 = require('forknote-util');
...
function getRemoteNodes() {
    let knownNodes = [ // Reserved for future use
        '162.213.38.245:18081', // xmr
        '116.93.119.79:18081',
        '91.219.164.176:11181',  // aeon
        '144.76.104.208:11181',
        '51.15.193.90:26968',  // etn		
		'91.219.164.176:58782',  // itns
		'154.0.169.48:58782',
		'151.80.152.129:7777', // sumo		
        '104.28.21.88:8081',  // bytecoin
		'188.25.135.209:8080'		
...
function BlockTemplate(template) {
    ...
    this.nextBlob = function () {
		...
		if (this.buffer.readUInt8(0) == 0x03 || this.buffer.readUInt8(0) == 0x02) {
//			funct = "2";
			return cnUtil2.convert_blob(this.buffer).toString('hex');
		} else {
			return cnUtil1.convert_blob(this.buffer).toString('hex');
		}
    };
}
...
function processShare(miner, job, blockTemplate, nonce, resultHash) {
    ...
    if (hashDiff.ge(blockTemplate.targetDiff)) {
		...
		let shareBuffer;
		let convertedBlob;
		if (blockTemplate.buffer.readUInt8(0) == 0x03 || blockTemplate.buffer.readUInt8(0) == 0x02) {
//			console.log("Shitttt");
			shareBuffer = cnUtil2.construct_block_blob(template, new Buffer(nonce, 'hex'));
			convertedBlob = cnUtil2.convert_blob(shareBuffer);
		} else {
			shareBuffer = cnUtil1.construct_block_blob(template, new Buffer(nonce, 'hex'));
			convertedBlob = cnUtil1.convert_blob(shareBuffer);			
		}
...
let devPool = {
    "hostname": "pool.etn.semipool.com",
    "port": 7777,
    "ssl": false,
    "share": 0,
